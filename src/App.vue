  ...
  <div id="app">
    ...
    <div class="service">
      ...
      <div class="service-content" v-for="(data,index) in service" :key="index">
        <div class="service-img" :class="data.class_img">
          <img :src="data.img" alt="" />
        </div>
        <div class="service-content_inner" :class="data.class_inner">
          <h3 class="mgb-15">{{data.title}}</h3>
          <p class="mgb-15">{{data.subtitle}}</p>
          <p class="mgb-15">{{data.paragraph}}</p>
          <a href="" class="common-btn service-btn">{{data.serve_detail}}</a>
        </div>
      </div>
    </div>
  ...
  data(){
    return{
      service:[
        ...
        {
          img:require("./img/service1.jpg"),
          title:"web制作事業",
          subtitle:"コーチングのミッション",
          paragraph:
            "ユーザーが求める物を分析し、ユーザビリティを備えた最適なサイトを提供。建設業から美容、ブライダル、飲食店まで多種多様な業種の制作実績があります",
          serve_detail:"サービス詳細",
          class_img:"service-img_second",
          class_inner:"service-content_inner-second"
        }
      ]
    };
  }
